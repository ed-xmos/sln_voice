...
void WriteRegs(int deviceAddr, int numDevices, int regAddr, int regData)
{
    ...
    {
        result = write_reg(&i2c_master_ctx, i, regAddr, regData);
        xassert(result == I2C_REGOP_SUCCESS); //I2C write reg failed
    }
}
...
void SetI2CMux(int ch)
{
    ...
    result = write_reg(&i2c_master_ctx, PCA9540B_I2C_DEVICE_ADDR, 0, ch);

    xassert(result == I2C_REGOP_SUCCESS); //I2C Mux I2C write reg failed
}
...
void AudioHwInit()
{
    ...
    else
    {
        ...
        result = write_reg(&i2c_master_ctx, PCM1865_0_I2C_DEVICE_ADDR, PCM1865_TX_TDM_OFFSET, 1);
        xassert(result == I2C_REGOP_SUCCESS); //ADC I2C write reg failed
        result = write_reg(&i2c_master_ctx, PCM1865_1_I2C_DEVICE_ADDR, PCM1865_TX_TDM_OFFSET, 129);
        xassert(result == I2C_REGOP_SUCCESS); //ADC I2C write reg failed
    ...
    else
    {
        ...
        for(int dacAddr = PCM5122_0_I2C_DEVICE_ADDR; dacAddr < (PCM5122_0_I2C_DEVICE_ADDR+4); dacAddr++)
        {
            ...
            result = write_reg(&i2c_master_ctx, dacAddr, PCM5122_I2S_SHIFT, 1 + (dacOffset * XUA_I2S_N_BITS * 2));
            xassert(result == I2C_REGOP_SUCCESS); //DAC I2C write reg failed
        }
    }
...
void AudioHwConfig(unsigned samFreq, unsigned mClk, unsigned dsdMode, unsigned sampRes_DAC, unsigned sampRes_ADC)
{
    ...
    if(CODEC_MASTER)
    {
        ...
        result |= write_reg(&i2c_master_ctx, dacAddr, PCM5122_DOSR, regVal);

        //# FS setting should be set based on sample rate
        regVal = samFreq/96000;
        result |= write_reg(&i2c_master_ctx, dacAddr, PCM5122_I16E_FS, regVal);

        //IDAC1  sets the number of miniDSP instructions per clock.
        regVal = 192000/samFreq;
        result |= write_reg(&i2c_master_ctx, dacAddr, PCM5122_IDAC_MS, regVal);

        /* Master mode setting */
        // BCK, LRCK output
        result |= write_reg(&i2c_master_ctx, dacAddr, PCM5122_BCK_LRCLK, 0x11);

        // Master mode BCK divider setting (making 64fs)
        regVal = (mClk/(samFreq * I2S_CHANS_PER_FRAME * XUA_I2S_N_BITS))-1;
        result |= write_reg(&i2c_master_ctx, dacAddr, PCM5122_DBCK, regVal);
        ...
        result |= write_reg(&i2c_master_ctx, dacAddr, PCM5122_DLRCK, regVal);

        // Master mode BCK, LRCK divider reset release
        result |= write_reg(&i2c_master_ctx, dacAddr, PCM5122_RBCK_LRCLK, 0x3f);
...
void hw_setup(void){
    ...
    AudioHwInit();
    AudioHwConfig(48000, MIC_ARRAY_CONFIG_MCLK_FREQ, 0, 32, 32);
}
